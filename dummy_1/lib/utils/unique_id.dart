...
class SafeIntIdCustom {
  ...
  SafeIntIdCustom({
    this.firstYear = 2023,
    Random? random,
    int randomValues = 100, // Adjust this value for a 9-digit range
  }) {
  ...
  /// Get a new uncoordinated unique 9-digit ID
  int get9DigitId() {
    final millis = DateTime.now().millisecondsSinceEpoch - _firstYearMillis;
    return (millis * randomValues + random.nextInt(randomValues)) % 1000000000;
  }
...
final safeIntId = SafeIntIdCustom();
